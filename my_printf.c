...
int my_printf(char* restrict format, ...)
{
    ...
    for(p = format; *p; p++)
    {
        ...
        switch (*++p) {
            ...
            case 'u':{
                value = va_arg(args_p, int);
                char* res_convert = itoa_unsigned_base(value, 10);
                int len = my_putstr(res_convert);
                written_chars += len;
                break;
            }
           case 'x':{
            value = va_arg(args_p, int);
            char* res_convert = itoa_unsigned_base(value, 16);
            int len = my_putstr(res_convert);
            written_chars += len;
            break;
        }
            ...
            case 'p':{
                ...
                res_convert = pointer_to_string(value_p, 18);
                write(1, "0x", 2);
                len = my_putstr(res_convert);
                written_chars += len + 2;
                break;
